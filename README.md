...
#### 1. 境界系統
```
1.0 練氣期 → 2.0 築基期 → 3.0 元嬰期 → 4.0 金丹期
→ 5.0 化神期 → 6.0 度劫期

每個大境界分為：初期 (x.0-x.2) → 中期 (x.3-x.5) → 後期 (x.6-x.9)
```
...
【修煉指令】c 或「修煉」
- 消耗：10 法力
- 獲得：10 修煉進度（靈池/內門 +50% 加成）
- 進度滿後可嘗試突破

【突破指令】b 或「突破」
- 條件：修煉進度 ≥ 所需值，HP ≥ 50%
- 成功率：基礎率 + 氣運加成（5%-95%）
- 成功：境界 +0.3，max_hp/mp 提升，完全恢復
- 失敗：受傷 (-30% max_hp)，進度減半
```

| 境界 | 所需進度 | 基礎成功率 | 突破獎勵 |
|------|---------|-----------|---------|
| 練氣期 | 100 | 70% | HP+10, MP+5 |
| 築基期 | 200 | 60% | HP+20, MP+10 |
| 元嬰期 | 400 | 50% | HP+40, MP+20 |
| 金丹期 | 800 | 40% | HP+60, MP+30 |
| 化神期 | 1600 | 30% | HP+100, MP+50 |
...
│   ├── keyword_tables.py  # 關鍵詞表管理 (65行)
│   ├── cultivation.py     # 境界修煉系統 (375行) ✨NEW
...
### v1.4.0 (2025-11-26) ✨ **境界突破制修煉系統**
🎮 **新功能**
- ✅ 新增 `cultivation.py` 境界修煉系統核心模組
- ✅ 新增修煉指令 `c`：消耗法力累積修煉進度
- ✅ 新增突破指令 `b`：嘗試境界突破
- ✅ 地點修煉加成（靈池/內門 +50%）
- ✅ 氣運影響突破成功率
...
🧪 **測試**
- 新增 22 個 cultivation 單元測試
- **測試總數：91 → 106**
- **通過率：93%**

📁 **檔案變更**
- `src/cultivation.py` - 新增
- `src/config.py` - 移除 experience/level，新增 breakthrough_attempts
- `src/main.py` - 新增 c/b 快捷指令
- `src/prompts.py` - 更新 AI prompts
- `src/game_state.py` - DB schema v3
- `tests/unit/test_cultivation.py` - 新增
